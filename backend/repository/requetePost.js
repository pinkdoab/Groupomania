...
Post.getAll = result => {
      ...
      connectionMySql.query("SELECT p_id AS id, p_texte AS texte, p_image_url AS imageUrl, p_date_creation AS dateCreation, t_user.u_pseudo AS createurPseudo,t_user.u_id AS createurId, count(o_favorable) AS nbAvisFavorable, count(o_defavorable) AS nbAvisDefavorable, t_user.u_moderateur AS moderateur FROM t_publication INNER JOIN t_user ON p_user_createur = u_id LEFT OUTER JOIN t_opinion ON p_id = o_commentaire GROUP BY p_id ORDER BY p_date_creation DESC;", (err, res) => {
...
Post.findById = (customerId, result) => {
  ...
  connectionMySql.query(`SELECT * FROM t_publication INNER JOIN t_user ON p_user_createur = u_id WHERE p_id = ${customerId}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
